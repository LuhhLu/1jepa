...
def save_kernels_as_images(conv3d_weight, grid_size=20, epoch=0):
    out_channels, in_channels, T, H, W = conv3d_weight.shape

    # 创建输出文件夹
    output_dir = f"conv3d_kernels_epoch_{epoch}"
...
def main(args, resume_preempt=False):
    ...
    for epoch in range(start_epoch, num_epochs):
        ...
        save_kernels_as_images(weight_quant(encoder.module.backbone.patch_embed.proj.weight.data.cpu()).numpy(),grid_size= 20 , epoch = epoch)
        # -- Save Last
